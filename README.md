...
|  类型  |  算法  |  适用场景  |  优点  |  缺点  |
| --- | --- | --- | --- | --- |
|  关键词  |  tf-idf  |  1.  文本分类      2.  信息检索      3.  特征提取       |  1.  简单直观，易于实现。      2.  能够反映词语在文档中的重要性。      3.  通过IDF部分减少了常见词的影响。       |  1.  不考虑词语的上下文信息。      2.  对于短文本，TF可能不够稳定。      3.  无法捕捉到词语之间的语义关系。       |
|  静态词向量  |  Word2Vec  |  1.  语义分析      2.  文本相似度计算      3.  词嵌入表示      4.  但在处理多义词和罕见词时效果较差。       |  1.  能够捕捉词语之间的语义关系。      2.  生成的词向量可以用于多种下游任务。      3.   静态词向量由于其固定不变，训练速度快，模型参数相对固定。       |  1.  训练完成后，词向量是静态的，无法适应新数据。      2.  无法很好地处理长距离依赖问题。      3.  在处理词语的多义性和上下文关系上存在局限。       |
|  动态词向量  |  Bert  |  1.  需要丰富上下文信息的任务语言理解任务，如如自然语言推理、问答系统或机器翻译等。      2.  作为预训练模型，微调用于特定任务。       |  1.  双向上下文理解，能够捕捉更丰富的语义信息。      2.  预训练模型可以在多种任务上进行微调，表现出色。      3.  通过注意力机制，能够更好地处理长距离依赖问题。       |  1.  模型参数多，计算量大，动态词向量需要实时计算，训练和推理速度慢。      2.  对于小数据集可能过拟合。       |
...
2.  测试集生成
    1.数据量：23年用户原声总计7万多条，问题量TOP10的产品有1万5千条，对应已沉淀的标准问约1350个
    2.  用户原声提取
        1. 问题分类：通过算法对每个产品下的问题聚类，得到12类用户问题：登录问题、数据问题、网关问题、权限问题等，其中，无法归类的统一分到其他问题中
        2. 抽取方式：按比例在不同分类中抽取10%的数据
    3.  数据标注：专业技术人员标注每个问题对应的标准问
    4.  处理结果：
        1.  问题频率：有的标准问可以有50个左右的用户原声，有的标准问就一两个用户原声，还有没有被问到的
        2.  问题形式：（用户原声，是否有标准问，标准问）

**测试过程**<br>
1.  使用测试集分别测试不同方法的匹配效果，记录匹配结果
    1. 精确率
    2. 召回率（TOP1 TOP3 TOP 5）
    3. F1值
2. 测试优化后的匹配效果
...
**bad case1**<br>
1.  问题描述：用户意图不清晰
2.  具体表现形式：XXX登录，无法确定用户咨询的是XXX怎么登录\登录失败等情况
3.  溯源：
4.  改进方案：制定问题表述明确的要求，接入大语言模型辅助判断用户意图是否明确，用户意图不明确时引导用户细化问题
5.  优化结果

**bad case2**<br>
1.  问题描述：用户问题中包含多个意图
2.  具体表现形式：登录失败，提示XXXX，重启后，XXX无法查询
3.  溯源：
4.  改进方案：
    1.  引导用户每次只提一个问题
    2.  用户query转换为多个子查询
...
**bad case3**<br>
1.  问题描述：用户query中的业务用词含义接近，但实际问题完全不同，解决方案也不同
2.  具体表现形式：故障/异常/失败
3.  溯源：现有算法中台能力限制，对语义的理解不够充分
4.  改进方案：算法中台提高语义理解的程度，增加rerank
5.  优化结果
   
**bad case4**<br>
1.  问题描述：用户query中的专有名词
2.  具体表现形式：web端/访客端/客户端/网页端/管理端
3.  溯源：
4.  改进方案：多路召回
5.  优化结果

**bad case5**<br>
1.  问题描述：标准问不明确（存在包含关系，存在相似问题，存在歧义）
2.  具体表现形式：
    1. 包含关系：XXX平台无法登录---XXX平台客户端无法登录
    2. 相似问题：数据异常，无法查询-----数据库异常，无法查询
    3. 歧义：超过最大用户数----平台的用户太多 or 账号登陆的设备太多
4.  溯源：问题边界不够清晰，没有明确规定标准问的写法和层次关系
5.  改进方案：
    1.  标准问去重
    2.  制定标准问编辑规范
        1.  标准问语义要明确简练，去掉无意义的词，只保留核心信息；明确出现问题的
        2.  标准问不包含多个意图，多个意图需要拆分
        3.  标准问多个问句为一个意图，需要合并
        4.  句型句式尽量保持规范，比如：是什么、为什么、怎么办
6.  优化结果
