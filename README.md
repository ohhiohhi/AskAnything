...
3.  准确度（主要通过重排序过程提高）：
    ...
    2.  $预测标准问和实际标准问一致的数量/模型根据用户query预测到的标准问的数量$<br>
**离线人工指标**<br>
...
2.  测试集生成
    1.  标准问选择要求
        1.  代表性：用户历史问题中总结出来的常见查询类型，共分为5类，不同类型问题的语法问法不同，评估模型对不同类型问题的理解能力
...
### 离线人工测试方案
### 测试过程
1.  使用测试集分别测试不同方法的匹配效果
2.  逐个比较系统返回的结果使用测试集分别测试不同方法的匹配效果
### 结果评估
1.  定义不同类问题使用的方法
### bad case识别与优化
1.  溯源
